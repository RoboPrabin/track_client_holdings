...
class ManagerSummaryExtractor:
    def extract_manager_summary(self):
        ...
        # holdings_path = r"D:\Trishakti\Projects\RPA\track_stock_price\data\output\holdings_FINAL.xlsx"
        df_holdings = pd.read_excel(holdings_path)

        # Step 2: Connect to PostgreSQL
        engine = create_engine(helper.get_engine_connection_intranet_db())
        # engine = create_engine("postgresql+psycopg2://postgres:admin@localhost:5432/trishakti_db")
        ...
        df_merged['broName'] = df_merged['rm_name'].fillna('N/A') + ' ' + df_merged['rm_fname'].fillna('')
